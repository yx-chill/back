<template>
  <div class="relative">
    ...
    <div v-if="showPopover" ref="popoverEl" class="mt-1.5 absolute top-full left-[-105px] md:left-auto w-[300px]
      ...
      <TabGroup>
        <TabList class="flex items-center space-x-2 px-3  pt-2.5 border-b border-gray-200">
          <Tab v-for="label in ['連結', '上傳圖片']" :key="label" as="template" v-slot="{ selected }">
            <button type="button" class="pb-1 text-sm tracking-wide border-b-2"
        ...
        <TabPanels class="px-3 py-3.5">
          <TabPanel class="flex items-center">
            <input type="text" placeholder="圖片連結..." class="form-input text-sm px-2 py-1"
          ...
          <TabPanel>
            <PrimaryButton class="btn-sm w-full flex items-center"
              @click="selectFile">
      ...
      <input type="file" accept="image/*" @change="onChangeFile" class="hidden" ref="fileEl">
    </div>
  </div>
</template>
...
export default {
  ...
  setup (props) {
    ...
    return { urlEl, url, showPopover, popoverEl, fileEl, handleAction, handleSubmit, selectFile, onChangeFile }
  },
}
</script>
